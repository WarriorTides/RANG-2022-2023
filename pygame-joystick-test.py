...
def translate_vector(vector):
    ...
    if abs(linearX) > 1:
        print("linearX out of range: %f" % linearX)
    
    if abs(linearY) > 1:
        print("linearY out of range: %f" % linearY)
    
    if abs(linearZ) > 1:
        print("linearZ out of range: %f" % linearZ)
    
    if abs(angularX) > 1:
        print("angularX out of range: %f" % angularX)

    T = [ # Vector of thrusts
        linearX + linearY + angularX,
        -linearX + linearY - angularX,
        linearX - linearY - angularX,
        -linearX - linearY + angularX,
        linearZ,
    ...
    # https://github.com/JHSRobo/rov_control/blob/6a4155740df6e02eac4bf430f853f7d9e0eaf57f/src/drive_control.py#L37

    max_motor = max(abs(x) for x in T)
